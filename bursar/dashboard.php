...
// ── Summary Stats ──────────────────────────────────────────────
$total_collected = $conn->query("SELECT COALESCE(SUM(amount_paid),0) AS t FROM fees")->fetch_assoc()['t'];
$total_outstanding = $conn->query("SELECT COALESCE(SUM(amount - amount_paid),0) AS t FROM fees WHERE status != 'paid'")->fetch_assoc()['t'];
$total_students   = $conn->query("SELECT COUNT(*) AS t FROM students")->fetch_assoc()['t'];
$unpaid_count     = $conn->query("SELECT COUNT(*) AS t FROM fees WHERE status='unpaid'")->fetch_assoc()['t'];
...
<title>Bursar Dashboard — AMUP</title>
...
:root{
  --maroon:#3d0a1a;--maroon2:#7B1C3E;--gold:#D4A825;--gold-light:#F0D060;
...
.nav-badge{margin-left:auto;font-size:10px;font-weight:700;padding:2px 7px;border-radius:20px;line-height:1.6;}
.nav-badge.green{background:#dcfce7;color:#16a34a;}
.nav-badge.red{background:#fee2e2;color:#dc2626;}
.nav-badge.yellow{background:#fef9c3;color:#b45309;}
...
.stat-card{background:var(--white);border-radius:16px;padding:24px;display:flex;align-items:center;gap:18px;border:2px solid rgba(61,10,26,0.06);box-shadow:0 2px 12px rgba(61,10,26,0.05);cursor:pointer;transition:transform .2s ease,box-shadow .2s ease,border-color .2s ease;text-decoration:none;color:inherit;}
.stat-card:hover{transform:translateY(-4px);box-shadow:0 10px 28px rgba(61,10,26,0.13);border-color:var(--maroon2);}
.stat-card.active-card{border-color:var(--maroon);background:#fdf5f7;box-shadow:0 6px 20px rgba(61,10,26,0.15);}
...
.stat-value{font-size:1.5rem;font-weight:700;color:var(--maroon);}
...
.btn{padding:9px 18px;border:none;border-radius:9px;font-size:13px;font-weight:600;cursor:pointer;font-family:inherit;display:inline-flex;align-items:center;gap:7px;}
...
<aside class="sidebar">
  ...
  <nav>
    ...
    <a href="dashboard.php" class="nav-link active"><i class="fas fa-chart-pie"></i> Overview</a>
    ...
    <a href="dashboard.php?status=paid&card=collected" class="nav-link <?= $active_card==='collected' ? 'active' : '' ?>">
      <i class="fas fa-check-circle"></i> Total Collected
      <span class="nav-badge green">₦<?= number_format($total_collected/1000,0) ?>k</span>
    </a>
    <a href="dashboard.php?status=unpaid&card=outstanding" class="nav-link <?= $active_card==='outstanding' ? 'active' : '' ?>">
      <i class="fas fa-exclamation-triangle"></i> Outstanding
      <span class="nav-badge red">₦<?= number_format($total_outstanding/1000,0) ?>k</span>
    </a>
    <a href="dashboard.php?status=unpaid&card=unpaid" class="nav-link <?= $active_card==='unpaid' ? 'active' : '' ?>">
      <i class="fas fa-clock"></i> Unpaid Records
      <span class="nav-badge yellow"><?= $unpaid_count ?></span>
    </a>
...
<div class="main">
  <div class="topbar">
    ...
    <div class="topbar-right">
      <a href="record_payment.php" class="btn btn-primary"><i class="fas fa-plus"></i> Record Payment</a>
...
<?php if (isset($_GET['success'])): ?>
  <div style="background:#dcfce7;color:#16a34a;padding:12px 20px;border-radius:10px;margin-bottom:20px;font-size:14px;font-weight:600;border:1px solid #bbf7d0;">
    <i class="fas fa-check-circle"></i> Payment updated successfully.
  </div>
<?php elseif (isset($_GET['error'])): ?>
  <div style="background:#fee2e2;color:#dc2626;padding:12px 20px;border-radius:10px;margin-bottom:20px;font-size:14px;font-weight:600;border:1px solid #fecaca;">
    <i class="fas fa-exclamation-circle"></i>
    <?php
      $msgs = [
        'invalid'  => 'Invalid fee record.',
        'notfound' => 'Fee record not found.',
        'overpaid' => 'Amount paid cannot exceed total fee amount.',
        'dbfail'   => 'Database error. Please try again.',
      ];
...
<?php endif; ?>
    ...
    <!-- Stats -->
    <div class="stats">
      <a href="dashboard.php?status=paid&card=collected" class="stat-card <?= $active_card==='collected' ? 'active-card' : '' ?>">
      ...
      <a href="dashboard.php?status=unpaid&card=outstanding" class="stat-card <?= $active_card==='outstanding' ? 'active-card' : '' ?>">
      ...
      <a href="dashboard.php?status=unpaid&card=unpaid" class="stat-card <?= $active_card==='unpaid' ? 'active-card' : '' ?>">
    ...
    <div class="table-wrap">
      <div class="table-header">
        ...
        <a href="summary.php" class="btn btn-success btn-sm"><i class="fas fa-file-export"></i> Export</a>
      </div>
      <table>
        <thead>
          <tr>
        ...
        <?php else: while($f = $fees->fetch_assoc()): ?>
          <tr>
            ...
            <td>
              <span class="badge badge-<?= $f['status'] ?>"><?= ucfirst($f['status']) ?></span>
            </td>
            <td>
  ...
  <div class="modal">
    ...
    <form method="POST" action="update_payment.php">
      ...
      <div class="form-group">
        ...
        <textarea name="remark" id="edit_remark" placeholder="Any additional notes..."></textarea>
      </div>
      <div class="modal-footer">
...
function openView(f) {
  ...
  document.getElementById('viewBody').innerHTML = `
    ...
    <div class="info-row"><span class="info-label">Department</span><span class="info-value">${f.department}</span></div>
